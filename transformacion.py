...
def enriquecer(
    ...
    """
    if ventas.empty:
        raise ValueError("❌ [Join] DataFrame de ventas vacío")
    ...
    print(
        f"✅ [Join] DataFrame maestro: {df.shape[0]:,} filas × {df.shape[1]} columnas"
    )
    return df
...
def segmentar(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "monto" not in df.columns:
        raise KeyError("❌ [Segmentación] Columna 'monto' no encontrada")
    if "edad" not in df.columns:
        raise KeyError("❌ [Segmentación] Columna 'edad' no encontrada")
    ...
    print("✅ [Segmentación]", df["segmento_cliente"].value_counts().to_dict())
    return df
...
def aplicar_pca(
    ...
    if not feats:
        raise ValueError("❌ [PCA] Ninguna feature válida encontrada en el DataFrame")
    if len(feats) < n:
        raise ValueError(
            f"❌ [PCA] Se necesitan al menos {n} features, "
            f"solo se encontraron {len(feats)}: {feats}"
        )

    sub = df[feats].dropna()
    if sub.empty:
        raise ValueError("❌ [PCA] No hay filas completas para aplicar PCA")
    ...
    print(f"✅ [PCA] Varianza explicada: {[f'{v * 100:.1f}%' for v in var]}")
...
def aplicar_kmeans(
    ...
    if cols_faltantes:
        raise KeyError(f"❌ [K-Means] Columnas faltantes: {cols_faltantes}")

    mask = df[cols_pc].notna().all(axis=1)
    if mask.sum() == 0:
        raise ValueError("❌ [K-Means] No hay filas completas en las PCs")

    km = KMeans(n_clusters=n_clusters, random_state=42, n_init="auto")
    df.loc[mask, "cluster"] = km.fit_predict(df.loc[mask, cols_pc])
    df["cluster"] = df["cluster"].fillna(-1).astype(int)

    print(f"✅ [K-Means] {n_clusters} clusters asignados")
    return df
